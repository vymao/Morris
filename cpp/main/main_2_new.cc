...
int main(int argc, char **argv)
{
    ...
    std::cout << input_tensor_values[3] << std::endl;
    ...
    std::queue<std::vector<float>> d_queue;
    d_queue.push(input_tensor_values);

    std::vector<Ort::Value> input_tensors = WakeClassifier::audioToValueVector(input_tensor_values, extractor);
    ...
    std::vector<float> expected_values_y = {0.000045f, 0.003846f, 0.000125f, 0.001180f, 0.001317f};
    std::cout << "Running model..." << std::endl;


    auto t1 = high_resolution_clock::now();
    //std::vector<Ort::Value> input = wake_classifier.prepareInputs(input_tensor_values);
    wake_classifier.runModelAsync(ort_outputs, d_queue);

...
def main() -> None:
    ...
    parser.add_argument("--ts-native-functions-path")
    ...
    if options.gen_lazy_ts_backend:
        ...
        ts_path = os.path.dirname(options.ts_native_functions_path)
        ts_backend_yaml = os.path.join(aten_path, "native/ts_native_functions.yaml")
        ts_native_functions = os.path.join(ts_path, "ts_native_functions.cpp")
        ts_node_base = os.path.join(ts_path, "torch/csrc/lazy/ts_backend/ts_node.h")
